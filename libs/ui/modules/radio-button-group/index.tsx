...
import { getIconSize, type Size } from '../../components/common';
...
const selectedClasses: Record<RadioButtonGroupColor, string> = {
  ...
  pending: 'bg-pending text-white hover:bg-pending-hover',
...
export function RadioButtonGroup<T extends string = string>({
  ...
  return (
    <div
      role="radiogroup"
      className={cn(
        'inline-flex items-stretch overflow-hidden rounded-lg border border-neutral-border bg-neutral-white',
        className,
        customClasses?.root,
      )}
    >
